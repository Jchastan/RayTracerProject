...
RT_PROGRAM void pathtracer() {
    ...
    a = fabsf(dot(a,w)) > .99f ? make_float3(1,0,0) : a;
    ...
    /*
    if (payload.depth > 0 && cf.NEE == 1 && attrib.objType == LIGHT) {
	payload.radiance = result;
    ...
    */
	...
	int whichLight = (k >= qlights.size()) ? k%qlights.size() : k;
		...
		if (k >= qlights.size()) {
			...
			Triangle tri = (poggers == 0) ? qlights[whichLight].tri1 : qlights[whichLight].tri2;

			float nDotWo = dot(tri.normal, -wi);

			float dist = dot(tri.v1 - attrib.intersection, tri.normal) / dot(wi, tri.normal);
			float3 P = attrib.intersection + dist * wi; // intersection in the object space
			...
			if (!(0 > u || u > 1 || 0 > v || v > 1 || u + v > 1 || nDotWo == 0.0f || dist < 0.001)) {
			    lightDir = wi;
			    light_dist = t;
			    sampled_light_pos = attrib.intersection + wi * dist;
			    shadow_payload.isVisible = true;
			    break;
			}
			else {
			...
			weight = powf(pdf_nee,2.0f)/(powf(pdf_nee,2.0f) + powf(pdf,2.0f));
			//rtPrintf("%f\n",brdf);
			...
			pdf_nee = (R*R)/(area*dot(n_light, lightDir))/qlights.size();
			weight = powf(pdf_nee,2.0f)/(powf(pdf_nee,2.0f) + powf(pdf,2.0f));
			//bruh *= weight * brdf * fmaxf(dot(n,lightDir),0) * (1.0f/pdf_nee);
			//sampled_result += weight * brdf * fmaxf(dot(n,lightDir),0) * (1.0f/pdf_nee);
			weight = powf(pdf,2.0f)/(powf(pdf_nee,2.0f) + powf(pdf,2.0f));
			//bruh *= weight * brdf * fmaxf(dot(n,lightDir),0) * (1.0f/pdf_nee);
			//sampled_result += weight * brdf * fmaxf(dot(n,lightDir),0) * (1.0f/pdf_nee);
		    }
		    else {
			sampled_result += brdf * G * (area / (float)light_samples);
		    }

                }
            }
        }
        L_d += qlights[whichLight].color * sampled_result;
    }



    //for (int k = 0; k < qlights.size(); k++) {
//	L_D +=  (qlights[k].pdf_nee/(qlights[k].pdf_nee + 
 //   }
